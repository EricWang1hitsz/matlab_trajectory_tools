function [] = drift_analysis(topic_config,... % T_IB (localization), T_JV (groundtruth localization)
                             T_VB,...
                             output_path,...
...
trajectory_viz_localization_symbol = '-m';
...
if(fast_groundtruth)
    [groundtruth_poses, localization_poses] = aligner.truncateAndResampleDatastreams(poses(2), poses(1));
else
    [localization_poses, groundtruth_poses] = aligner.truncateAndResampleDatastreams(poses(1), poses(2));
end
groundtruth_poses.applyStaticTransformRHS(T_VB);

%% RMSE analysis

T_alignment = aligner.calculateAlignmentTransform(localization_poses,...
                                                  groundtruth_poses,...
                                                  alignment_orientation_weight);
groundtruth_poses_aligned = groundtruth_poses.applyStaticTransformLHS(T_alignment);

position_rmse = rms(cellfun(@norm, num2cell((groundtruth_poses_aligned.positions - localization_poses.positions)*100, 2))); %cm
orientation_rmse = rms(cellfun(@k_quat_diff_mag,...
                       num2cell(groundtruth_poses_aligned.orientations, 2),...
                       num2cell(localization_poses.orientations, 2))*180/pi); %deg
...
plot3(localization_poses.positions(:,1),...
      localization_poses.positions(:,2),...
      localization_poses.positions(:,3), trajectory_viz_localization_symbol);
plot3(localization_poses.positions(1,1),...
      localization_poses.positions(1,2),...
      localization_poses.positions(1,3), 'go');
plot3(localization_poses.positions(end,1),...
      localization_poses.positions(end,2),...
      localization_poses.positions(end,3), 'ro');
axis equal
grid on
xlabel('_{I}x [m]');
ylabel('_{I}y [m]');
...
legend([lineobjects(3) lineobjects(6)],{[topic_config(1).pose_id ' localization'],[topic_config(2).pose_id ' ground truth']},...
